...
public class CovMatrixPane extends JScrollPane {
	...
	///////////////////////////////
	// Constructor
	public CovMatrixPane(MatrixViewMainPane viewer) {
		super();
...
//		return (ColorMapper) this.getViewer().getColoringPane().getMatrixColoringModel().getSelectedItem();
	...
	private class ImagePanel extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			if (CovMatrixPane.this.data!=null) {
				if (CovMatrixPane.this.getImage()==null) {
					CovMatrixPane.this.setImage(CovMatrixPane.this.createMapImage());
				}
				g.drawImage(CovMatrixPane.this.getImage(), 0, 0, null);
				
				if (CovMatrixPane.this.showArea) {
					
					
					((Graphics2D)g).setColor(Color.green);
					((Graphics2D)g).draw(CovMatrixPane.this.area);
	...
	private class GetValuesFromPointMouseListener implements MouseListener, MouseMotionListener {
		...
		private void exportData(int mpx,int mpy) {
			...
			int px = mpx - 30;
			int py = mpy - 30;
			px = Math.min(CovMatrixPane.this.data.getData().getSize() - CovMatrixPane.ZOOM_SIZE, px);
			py = Math.min(CovMatrixPane.this.data.getData().getSize() - CovMatrixPane.ZOOM_SIZE, py);
			px = Math.max(0, px);
			py = Math.max(0, py);
			int w = Math.min(CovMatrixPane.ZOOM_SIZE, CovMatrixPane.this.data.getData().getSize() - px);
			int h = Math.min(CovMatrixPane.ZOOM_SIZE, CovMatrixPane.this.data.getData().getSize() - py);
			rect.setBounds(px, py, w, h);
			
			double[][] values = new double[w][h];
			char[] hChars = Arrays.copyOfRange(CovMatrixPane.this.getAminoAcids(),px,px+w);
			char[] vChars = Arrays.copyOfRange(CovMatrixPane.this.getAminoAcids(),py,py+h);
			for (int i = 0; i< w;i++) {
				for (int j =0 ; j<h;j++) {
					double value = CovMatrixPane.this.data.getData().getZscoreValue(1+px+i, 1 + py + j);
					values[i][j] = value;
				}
			}
			CovMatrixPane.this.getViewer().zoomArea(rect,values,hChars,vChars);
			
			Rectangle areaRect = new Rectangle();
			areaRect.setBounds(rect.x+30, rect.y+30, w, h);
			
			
			CovMatrixPane.this.area = areaRect;
			CovMatrixPane.this.showArea = true;
			CovMatrixPane.this.updateUI();
		}
		...
		public void mouseMoved(MouseEvent e) {
			int mpx = e.getX();
			int mpy = e.getY();
			if (CovMatrixPane.this.getMatrix()!=null) {
				exportData(mpx,mpy);
			}
		}
